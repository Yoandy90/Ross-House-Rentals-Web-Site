...
/** Simple Markdown-ish renderer for legal docs */
    ...
    <View>
      {lines.map((line, i) => {
        ...
        if (trimmed.startsWith('# ')) {
          return (
            <Text key={i} style={styles.h1}>{trimmed.replace(/^# /, '')}</Text>
          );
        }
        // H2
        if (trimmed.startsWith('## ')) {
          return (
            <Text key={i} style={styles.h2}>{trimmed.replace(/^## /, '')}</Text>
          );
        }
        // H3
        if (trimmed.startsWith('### ')) {
          return (
            <Text key={i} style={styles.h3}>{trimmed.replace(/^### /, '')}</Text>
          );
        }
        // Bullet
        if (trimmed.startsWith('- ')) {
        ...
        // Regular paragraph
        return (
          <Text key={i} style={styles.paragraph}>{renderBold(trimmed)}</Text>
        );
      })}
    </View>
  );
...
/** Render **bold** markers within text */
