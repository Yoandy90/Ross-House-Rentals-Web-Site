...
export default function PrivacyScreen() {
  ...
  const fetchPrivacy = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_URL}/legal?slug=privacy&lang=${lang}`);
      if (r.ok) {
  ...
  return (
    <>
      <Stack.Screen options={{ title: t('privacy.title', 'Política de Privacidad') }} />
      <SafeAreaView style={S.container} edges={['bottom']}>
        {loading ? (
          ...
          <ScrollView contentContainerStyle={S.scroll}>
            <Text style={S.heading}>{t('privacyContent.heading', 'Privacy Policy')}</Text>
            <Text style={S.date}>{t('privacyContent.updated', 'Last updated: January 2026')}</Text>
            <Text style={S.body}>{t('privacyContent.s1b', 'We protect your personal information with bank-grade encryption.')}</Text>
