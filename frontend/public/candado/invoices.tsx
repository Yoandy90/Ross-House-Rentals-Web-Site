...
export default function InvoicesScreen() {
  ...
  const downloadPDF = async (invoice: Invoice) => {
    try {
      ...
      if (!token) {
        Alert.alert(t('common.error'), 'No autorizado');
        return;
      }

      const pdfUrl = `${BACKEND_URL}/api/invoices/${invoiceId}/pdf?token=${encodeURIComponent(token)}`;
      await Linking.openURL(pdfUrl);
    } catch (error: any) {
      console.error('Error downloading PDF:', error);
      Alert.alert(t('common.error'), 'Error al descargar PDF');
    } finally {
        ...
        <View style={styles.cardContent}>
            ...
            <View style={[styles.statusBadge, { backgroundColor: statusColor + '15' }]}>
              ...
              <Text style={[styles.statusText, { color: statusColor }]}>
                {item.status === 'paid' ? 'Pagada' : 
                 ...
                 item.status === 'overdue' ? 'Vencida' : 'Cancelada'}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.actionsRow}>
            ...
            {(item.status === 'pending' || item.status === 'overdue') && (
              <TouchableOpacity
                style={[styles.payButton, item.status === 'overdue' && styles.payButtonUrgent]}
                onPress={() => handlePayInvoice(item)}
              >
                <Ionicons name="card-outline" size={18} color="#fff" />
                <Text style={styles.payButtonText}>
                  {item.status === 'overdue' ? 'Pagar Ahora' : 'Pagar'}
                </Text>
              </TouchableOpacity>
            )}
          </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.filtersContainer}>
        <FilterButton filter="all" label="Todas" count={stats.total} />
        <FilterButton filter="pending" label="Pendientes" count={stats.pending} />
        <FilterButton filter="paid" label="Pagadas" count={stats.paid} />
        <FilterButton filter="overdue" label="Vencidas" count={stats.overdue} />
      </View>
